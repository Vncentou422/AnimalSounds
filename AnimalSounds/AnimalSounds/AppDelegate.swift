...
class AppDelegate: UIResponder, UIApplicationDelegate, UISplitViewControllerDelegate {
    ...
    var dataFileName = "SoundCollection"
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplicationLaunchOptionsKey: Any]?) -> Bool {
        ...
        if let masterViewController = masterNavController.topViewController as? MasterViewController {
            masterViewController.objects = soundCollection
        }
    ...
    func saveData() {
        NSKeyedArchiver.archiveRootObject(soundCollection, toFile: fileURL.path)
    }
    
    func loadData() {
        if let items = NSKeyedUnarchiver.unarchiveObject(withFile: fileURL.path) as? SoundCollection {
            soundCollection = items
        } else {
